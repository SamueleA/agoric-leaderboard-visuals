        ...
        axios
            ...
            .then(async res => {
                ...
                for (const dataObj of delegatorArray) {
                    newAxiosRequest.push(`http://139.59.67.100:1317/cosmos/distribution/v1beta1/delegators/${dataObj}/rewards`)
                }
                await axios.all(newAxiosRequest.map(l => axios.get(l))).then(
                    axios.spread((...responses) => {
                            for (const dataObj of responses) {
                                let res = dataObj.config.url;
                                res = res.match(/agoric.*r/);
                                res = res[0];
                                res = res.substring(0, res.length - 6)
                                console.log("result", res)

                                if (dataObj.data.total.length > 0) {
                                    combinedArray.push(
                                        {
                                            value: parseFloat(dataObj.data.total[0].amount).toFixed(2),
                                            key: (res)
                                        }
                                    )
                                }
        ...
        <div className="App">
            <div>
                <Bar
                    ...
                    options={{
                        responsive: true,
                        title: {text: "Top 10 Delegators by Most Rewards Earned", fontSize: 25, display: true},
                        scales: {
                            yAxes: [
                                {
                                    ticks: {
